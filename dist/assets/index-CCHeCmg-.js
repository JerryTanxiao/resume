(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const e of document.querySelectorAll('link[rel="modulepreload"]'))d(e);new MutationObserver(e=>{for(const t of e)if(t.type==="childList")for(const r of t.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&d(r)}).observe(document,{childList:!0,subtree:!0});function s(e){const t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin==="use-credentials"?t.credentials="include":e.crossOrigin==="anonymous"?t.credentials="omit":t.credentials="same-origin",t}function d(e){if(e.ep)return;e.ep=!0;const t=s(e);fetch(e.href,t)}})();document.addEventListener("DOMContentLoaded",()=>{const m=new IntersectionObserver(i=>{i.forEach((c,n)=>{c.isIntersecting&&setTimeout(()=>{c.target.classList.add("visible")},n*100)})},{threshold:.1});document.querySelectorAll(".section, .glass-card, .timeline-item").forEach(i=>{i.style.opacity="0",i.style.transform="translateY(30px)",i.style.transition="all 1s cubic-bezier(0.19, 1, 0.22, 1)",m.observe(i)});const o=document.querySelector(".cursor-blob");let s=0,d=0,e=0,t=0;document.addEventListener("mousemove",i=>{s=i.clientX,d=i.clientY});function r(){e+=(s-e)*.1,t+=(d-t)*.1,o&&(o.style.transform=`translate(${e-200}px, ${t-200}px)`),requestAnimationFrame(r)}r();const a=document.getElementById("project-modal"),u=document.getElementById("modal-body"),v=document.querySelector(".close-modal"),y={ddms:{title:"DDMS 智能配送管理 SaaS (2.0 AI版)",desc:"已覆盖 **180 个区域代理商**。除高并发订单分发外，重点落地了 **AI Agent** 辅助运营与全链路消息积压深度治理方案。",tech:["Java 21","AI Agent","AliCloud MQ","Netty","Seata"],details:`
            ...
            `},wuwei:{title:"无为教育金融信息化系统 (10亿级营收支撑)",desc:"技术组长（管理 15+ 研发团队）。由于私有化部署限制，利用 **时间轮算法** 对 RocketMQ 进行了源码级二次开发，实现自主可控的延迟队列。",tech:["RocketMQ 源码开发","时间轮算法","JProfiler","Python 自动化","10w+ 在线直播"],details:`
                <div class="modal-detail-grid">
                    <div class="detail-item">
                    ...
                    <div class="detail-item">
                        <h4>微服务平滑治理</h4>
                        <p>主导单体 CRM 向微服务（直播/支付/营销）平滑剥离。利用 JProfiler 定位 OOM 根因，实现系统弹性伸缩能力。</p>
                    </div>
                </div>
            `},xiudian:{title:"咻电员工管理 & 商城 SaaS 生态",desc:"技术组长。主导千万级消息积压专项治理，确保高并发 IoT 核心业务与自动清分系统的稳定性。",tech:["Netty","Protobuf","AliCloud MQ","Kettle","轨迹纠偏"],details:`
                <div class="modal-detail-grid">
                    <div class="detail-item">
                    ...
                    <div class="detail-item">
                        ...
                        <p>基于 Netty + Protobuf 实现。针对边缘信号引入 **经纬度压缩平滑纠偏算法**，大幅缩减误差，解决轨迹分叉难题。</p>
                    </div>
                    <div class="detail-item">
                        <h4>业务自动化演进</h4>
                        <p>独立实现每日 50w 级的自动清分派单工单系统；负责 JVM 调优与 Kettle 数据同步链路建设。</p>
                    </div>
                </div>
            `}};document.querySelectorAll(".project-card").forEach(i=>{i.addEventListener("click",c=>{c.preventDefault();const n=i.getAttribute("data-project"),l=y[n];l&&(u.innerHTML=`
                ...
                ${l.details}
            `,a.style.display="block",document.body.style.overflow="hidden")})}),v.onclick=()=>{a.style.display="none",document.body.style.overflow="auto"},window.onclick=i=>{i.target===a&&(a.style.display="none",document.body.style.overflow="auto")};const p=document.createElement("style");p.innerHTML=`
        .visible {
            opacity: 1 !important;
